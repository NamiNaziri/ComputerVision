...
ImageSourcePath = 'Questions/Q3Helper/';
...
testImage = imcomplement(im2double(FeatureDataset{41,3})) ;
...
for i = 1 : min(size(xMin,2),size(xMax,2))
    for j = 1 : min(size(yMin,2),size(yMax,2))
        ...
        if sum(cropedImage(:)) > 5
            ...
            f = FeatureDataset{41,4}(yMin(j) +6 + yMin1: yMax(j)+6 - (cropedImageHeight - yMax1) ,xMin(i)+6 + xMin1: xMax(i)+6 -(cropedImageWidth - xMax1),:);
